...
def preprocess_retail(ds=None, **kwargs):
    ts = kwargs["ts_nodash"]  # ← DAG 実行時刻 (例: 20250912T182530)
...
def calc_retail_metrics(ds=None, **kwargs):
    ts = kwargs["ts_nodash"]
    df = pd.read_csv(f"{OUTPUT_DIR}/retail_clean_{ts}.csv", parse_dates=["date"])
...
def calc_ads_metrics(ds=None, **kwargs):
    ts = kwargs["ts_nodash"]
    df = pd.read_csv(f"{OUTPUT_DIR}/ads_clean_{ts}.csv")
    metrics = {
        "avg_CTR": df["CTR"].mean(),
